...
interface EditorSegmentProps {
  ...
  handlers: {
    ...
    onSave: (segmentUpdate: SegmentUpdateDTO) => void;
  };
}
...
export function EditorSegment({ data, handlers }: EditorSegmentProps) {
  ...
  const [localTargetText, setLocalTargetText] = useState(targetText);
  const [localStatus, setLocalStatus] = useState(status ?? "untranslated");
  ...
  return (
    ...
    >
      ...
      />
      <TranslationStatus
        isCompleted={localStatus === "translated"}
        onClick={() =>
          setLocalStatus((prevStatus) =>
            prevStatus === "translated" ? "untranslated" : "translated"
          )
        }
        onBlur={() =>
          handlers.onSave({
